...
?>
<div class="personal_info col-xs-12">
	<div class="profile_img text-center col-xs-3">
		<?php if ($proprio_perfil) { ?><span class="glyphicon glyphicon-cog text-left" data-toggle="modal" data-target="#definicoes" style="font-size: 30px"></span><?php }?>
		<img  class="img-responsive" style="border-radius: 5px" src="<?php echo $user_info["profile_img"]?>" width="250"/>
		<a href="mailto:<?php echo $user_info["email"]?>" style="color: lightblue" ><h3><?php echo $user_info["nome"]?></h3></a>
		<?php if ($proprio_perfil) { ?>
	<!-- Modal para as Definições -->
	<form action="" method="post" class="col-xs-12 text-left">
	...
	<?php if ($proprio_perfil) { ?><button class="btn btn-success" data-toggle="modal" data-target="#adicionar_concerto">
		<span class="glyphicon glyphicon-plus" style="color: white;" aria-hidden="true"></span>
				...
				<!-- Fim do Modal -->
				<?php $concertos = mysql_fetch_array(mysql_query("SELECT user_id, concertos FROM Utilizadores WHERE user_id={$id}"))["concertos"];
...
<ul class=" nav nav-tabs navbar-left">
	...
	?>
	<li class="<?php echo (($no_view || $view != 'Perguntas' && $view != 'Respostas') ? 'active' : 'inactive' )?>">
